...
type filterTypes = "colors" | "priceRange" | "categories"
...
export function FilterSidebar({ onFiltersChange }: FilterSidebarProps) {
  ...
  const handleColorChange = (colorId: string, checked: boolean) => {
    const newColors = checked ? [...selectedColors, colorId] : selectedColors.filter((id) => id !== colorId)
  ...
  const handlePriceChange = (value: number[]) => {
    setPriceRange(value)
    onFiltersChange?.({ categories: selectedCategories, priceRange: value, colors: selectedColors })
  }
            ...
            {categoriesLoading ? (
              ...
              categories.map((category) => (
                <div key={category.id} className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor={category.id} className="text-sm text-gray-700 dark:text-silver cursor-pointer flex-1">
                    {category.label}
                  </label>
